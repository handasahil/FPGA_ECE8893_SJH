    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IN_TRIS_DATA_0: begin
                    rdata <= int_in_tris[31:0];
                end
                ...
                ADDR_OUT_PIXELS_DATA_1: begin
                    rdata <= int_out_pixels[63:32];
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IN_TRIS_DATA_0)
            int_in_tris[31:0] <= (WDATA[31:0] & wmask) | (int_in_tris[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_out_pixels[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_PIXELS_DATA_0)
            int_out_pixels[31:0] <= (WDATA[31:0] & wmask) | (int_out_pixels[31:0] & ~wmask);
    end
end

// int_out_pixels[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_out_pixels[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_PIXELS_DATA_1)
            int_out_pixels[63:32] <= (WDATA[31:0] & wmask) | (int_out_pixels[63:32] & ~wmask);
    end
end
