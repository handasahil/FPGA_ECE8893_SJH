...
`ifndef TOP_KERNEL_ENV__SV                                                                                   
    ...
    class top_kernel_env extends uvm_env;                                                                          
        ...
        axi_pkg::axi_env#(64,4,8,3,1) axi_master_in_r;
        axi_pkg::axi_env#(64,4,8,3,1) axi_master_out_r;
    ...
    function void top_kernel_env::build_phase(uvm_phase phase);                                                    
        ...
        top_kernel_cfg.in_r_cfg.set_default();
        top_kernel_cfg.in_r_cfg.drv_type = axi_pkg::SLAVE;
        top_kernel_cfg.in_r_cfg.reset_level = axi_pkg::RESET_LEVEL_LOW;
        top_kernel_cfg.in_r_cfg.write_latency_mode = TRANSACTION_FIRST;
        top_kernel_cfg.in_r_cfg.read_latency_mode = TRANSACTION_FIRST;
        uvm_config_db#(axi_pkg::axi_cfg)::set(this, "axi_master_in_r*", "cfg", top_kernel_cfg.in_r_cfg);
        axi_master_in_r = axi_pkg::axi_env#(64,4,8,3,1)::type_id::create("axi_master_in_r", this);

        top_kernel_cfg.out_r_cfg.set_default();
        top_kernel_cfg.out_r_cfg.drv_type = axi_pkg::SLAVE;
        top_kernel_cfg.out_r_cfg.reset_level = axi_pkg::RESET_LEVEL_LOW;
        top_kernel_cfg.out_r_cfg.write_latency_mode = TRANSACTION_FIRST;
        top_kernel_cfg.out_r_cfg.read_latency_mode = TRANSACTION_FIRST;
        uvm_config_db#(axi_pkg::axi_cfg)::set(this, "axi_master_out_r*", "cfg", top_kernel_cfg.out_r_cfg);
        axi_master_out_r = axi_pkg::axi_env#(64,4,8,3,1)::type_id::create("axi_master_out_r", this);
    ...
    function void top_kernel_env::connect_phase(uvm_phase phase);
        ...
        if(top_kernel_cfg.in_r_cfg.drv_type==axi_pkg::MASTER ||top_kernel_cfg.in_r_cfg.drv_type==axi_pkg::SLAVE)
            top_kernel_virtual_sqr.in_r_sqr = axi_master_in_r.vsqr;
        axi_master_in_r.item_wtr_port.connect(subsys_mon.in_r_wtr_imp);
        axi_master_in_r.item_rtr_port.connect(subsys_mon.in_r_rtr_imp);
        uvm_callbacks#(axi_pkg::axi_state, axi_pkg::axi_state_cbs)::add(axi_master_in_r.state, refm.axi_memaccess_cb_in_r);
        if(top_kernel_cfg.out_r_cfg.drv_type==axi_pkg::MASTER ||top_kernel_cfg.out_r_cfg.drv_type==axi_pkg::SLAVE)
            top_kernel_virtual_sqr.out_r_sqr = axi_master_out_r.vsqr;
        axi_master_out_r.item_wtr_port.connect(subsys_mon.out_r_wtr_imp);
        axi_master_out_r.item_rtr_port.connect(subsys_mon.out_r_rtr_imp);
        uvm_callbacks#(axi_pkg::axi_state, axi_pkg::axi_state_cbs)::add(axi_master_out_r.state, refm.axi_memaccess_cb_out_r);
